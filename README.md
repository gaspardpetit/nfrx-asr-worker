...
- The worker now runs `worker.concurrency` jobs concurrently in one process; default is `4`, configurable via `worker.concurrency`, `NFRX_CONCURRENCY`, or `--concurrency`.
...
- `output.formats: [jsonl]` now returns one JSON object per line with `speaker`, `start`, `end`, and `text`.
